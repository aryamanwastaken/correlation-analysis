...
class StockData:
    def __init__(self, symbols, ndays):
        self.symbols = symbols
        self.start_date = self.calculate_start_date(ndays)
        self.end_date = pd.Timestamp.now().tz_localize(tzlocal()).strftime("%Y-%m-%d")
    ...
    def fetch_historical_data_v2(self, timeframe=TimeFrame.Day, limit=10000):
        ...
        while True:
            ...
            for symbol, bars in response_json["bars"].items():
                ...
                df["symbol"] = symbol
                df["t"] = pd.to_datetime(df["t"]).dt.tz_convert(tzlocal())
...
class CorrelationMatrix:
    ...
    def compute_correlation_matrix(self):
        """Compute the correlation matrix for the given data and column."""
        data = self.data.copy()
        data.set_index("t", inplace=True)
        df_pivot = data.pivot(columns="symbol", values=self.column)
        df_zscore = (df_pivot - df_pivot.mean()) / df_pivot.std()
        self.correlation_matrix = df_zscore.corr()

    def sort_correlation_matrix(self, sort_by="mean"):
        """Sorts the correlation matrix based on a specified metric (max, min, mean, median, or sum)."""
        metrics = {
            "max": self.correlation_matrix.max(),
            "min": self.correlation_matrix.min(),
            "mean": self.correlation_matrix.mean(),
            "median": self.correlation_matrix.median(),
            "sum": self.correlation_matrix.sum(),
        }
        ...
        sorted_correlation_matrix = self.correlation_matrix.reindex(
            index=sorted_columns, columns=sorted_columns
        )

        return sorted_correlation_matrix, metric

    def plot_correlation_matrix(self, sorted_correlation_matrix, colormap, title="Correlation Matrix"):
        """Plot the correlation matrix."""
        f = plt.figure(figsize=(12, 13))
        plt.matshow(
            sorted_correlation_matrix, fignum=f.number, cmap=plt.get_cmap(colormap)
        )
        plt.xticks(
            range(sorted_correlation_matrix.shape[1]),
            sorted_correlation_matrix.columns,
            fontsize=7,
            rotation=45,
        )
        plt.yticks(
...
class TimeSeriesPlots:
    def __init__(self, data, sorted_symbols, sort_by, metric, column):
        self.data = data
    ...
    def plot_time_series(self):
        ...
        # Use pivot_table and aggregate duplicates (if any) by taking their mean
        data_pivot = self.data.pivot_table(index='t', columns="symbol", values=self.column, aggfunc='median')

        # Reorder the columns based on the sorted symbols
        data_pivot = data_pivot[self.sorted_symbols]
        ...
        fig.text(
            0.5,
            0.98,
            f"Cross-correlation Time-series Sorted by {self.sort_by} (Based on {self.column} column)",
            ha="center",
            va="top",
            fontsize=14,
        )
        ...
        for i, ax in enumerate(axs.flatten()):
            if i < list_length:
                ...
                sort_value = round(self.metric[data_pivot.columns[i]], 3)
                ax.set_title(
                    f"{data_pivot.columns[i]} ({self.sort_by}: {sort_value})",
                    fontsize=8,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Method for sorting the correlation matrix (default=median)",
    )
    parser.add_argument(
        "--column",
        "-c",
    ...
    with open(arguments.list, "r") as f:
        symbols = [line.strip().upper() for line in f]

    # Instantiate the StockData class and fetch the data
    stock_data = StockData(symbols, arguments.ndays)
    stock_data.fetch_historical_data_v2()
    data = stock_data.data

    print ( data )

    # Instantiate the CorrelationMatrix class
    correlation_matrix_obj = CorrelationMatrix(data, arguments.column)

    # Compute the correlation matrix
    correlation_matrix_obj.compute_correlation_matrix()

    # Sort the correlation matrix
    sorted_correlation_matrix, metric = correlation_matrix_obj.sort_correlation_matrix(arguments.sort)

    # Plot the sorted correlation matrix
    correlation_matrix_obj.plot_correlation_matrix(
        sorted_correlation_matrix,
        colormap=arguments.colormap,
        title=f"Stocks Correlation Matrix (Based on {arguments.column} column)"
    )

    # Instantiate the TimeSeriesPlots class and plot time series of each stock
    time_series_plots = TimeSeriesPlots(data, sorted_correlation_matrix.columns, arguments.sort, metric, arguments.column)
